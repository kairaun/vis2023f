...
FileAttachment("simple(分數).json").json()
)}
...
function _simple(d3,simple1,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 600;
  const height = 400;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(simple1);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["simple(分數).json", {url: new URL("../output.json", import.meta.url), mimeType: "application/json", toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], _1);
  main.variable(observer("simple1")).define("simple1", ["FileAttachment"], _simple1);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("simple")).define("simple", ["d3","simple1","drag","invalidation"], _simple);
  return main;
}
